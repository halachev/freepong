$(document).ready(function () {
	...
	function onDeviceReady() {	
		...
		document.addEventListener("onAdDismiss", onAdClosed);
	...
	function showLoading(_message) {
	
		progress.show(_message);
	...
	function InitAdMob() {
		...
		if (AdMob) {
			...
			admob_android_key = 'ca-app-pub-2108590561691007/2190161573';
			...
			AdMob.prepareInterstitial( {adId:interstitial, autoShow:false} );		
	...
	function startGame()
	{
		...
		ball = {
			...
			update: function() {
				...
				if (0 > this.x+this.side || this.x > WIDTH) {
					...
					{
						let audio = new Audio('eatpill.mp3');
						audio.play();
					}
			...
			draw: function() {
				...
				var radius = 10;
				ctx.beginPath();
				ctx.arc(this.x, this.y, radius, this.side, this.side * Math.PI, false);								
				ctx.fill();
				ctx.lineWidth = 1;
		...
		function main() {
			...
			d.innerHTML = '<svg onclick="requestFullscreen()"width="20" height="16" opacity="0.5"><path d="M0 5v-5h5m10 0h5v5m0 6v5h-5m-10 0h-5v-5M6 6h8v4h-8z"style="fill:none;stroke:#f5f5f5;stroke-width:4"></path></svg>';
